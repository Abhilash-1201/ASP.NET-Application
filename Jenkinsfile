        ...
        stage('Slack Notification') {
            steps {
                script {
                   ...
                   def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://18.219.150.56:9000/api/qualitygates/project_status?projectKey=dotnet" | jq -r .projectStatus.status').trim()
