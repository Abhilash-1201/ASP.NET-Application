    ...
    stages{
        stage('code checkout from GitHub'){
            steps{
                ...
                git 'https://github.com/Abhilash-1201/ASP.NET-Application.git'
                //script {
        ...
        stage('Slack Notification') {
            steps {
                script {
                    ...
                    def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://18.219.150.56:9000/api/qualitygates/project_status?projectKey=nodejsproject" | jq -r .projectStatus.status').trim()
                    
                   if (qg == 'ERROR') {
                      slackSend color: '#FF0000', message: 'SonarQube Analysis failed. View the report at\n\nSonarQube Analysis Report : http://18.224.3.77:9000/dashboard?id=maven\n\nGuest Username: guest01\n\nGuest Password: guest01'
                  }
                  else if (qg == 'OK') {
